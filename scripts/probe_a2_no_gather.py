...
def main():
    ...
    # Add bias at bit-rev positions
    bias_pad = [0.0] * be._num_slots
    for j in range(h):
        bias_pad[bitrev(j)] = b1[j]
    ct = be.add_plain(ct, bias_pad)

    # Sanity: z1 should be at slot[bitrev(j)]
    dec = be.decrypt(ct)
    err_z1 = float(np.max(np.abs(np.array([dec[bitrev(j)] for j in range(h)]) - z1)))
    print(f"  z1 at bit-rev slots? err={err_z1:.3e}")
