"""Phase 8i: L=64 BERT-base bench using STREAMING plan (chunked encoding).

Memory-feasible at N=2^17 with depth-22 chain — pre-eager planner OOM'd
at ~50 GB plaintext footprint per matrix.
"""
import gc, time
import numpy as np
...
def main():
    ...
    def lin(name, x_in, W, bias=None):
        ...
        d_in = be._ops.depth(x_in)
        t = time.time()
        plan = build_colmajor_linear_plan_streaming(
            be, W, L=L, in_dim=in_d, out_dim=out_d, bias=bias, ct_depth=d_in,
        )
        stamp_build(name, time.time() - t)
        t = time.time()
        Y = linear_colmajor_streaming(be, x_in, plan)
    ...
    print()
    print("=" * 70)
    print(f"L={L} STREAMING per-layer  hidden={hidden}  N={N}")
    print("=" * 70)
    ...
    print(f"  plan-build (one-off) {bt*1000:8.1f} ms")
