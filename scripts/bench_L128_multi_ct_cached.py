"""Phase 8k: full L=128 BERT-base 2-layer bench at N=2^16 with plan cache.

Measures both the cold-cache (layer 1) and warm-cache (layer 2) per-layer
costs, then projects 12-layer e2e using the warm number for layers 2..12.
"""
import gc, time
import numpy as np

from fhe_thesis.encryption.heongpu_backend import HEonGPUBackend
from fhe_thesis.encryption.ops_attention_nexus import (
    pack_colmajor_multi,
    qk_scores_nexus, attn_apply_nexus,
    layernorm_colmajor_multi,
    linear_colmajor_multi_streaming,
    add_multi,
    prepare_colmajor_keys,
)


def log(m): print(f"[{time.strftime('%H:%M:%S')}] {m}", flush=True)


def main():
    L = 128
    hidden = 768
    num_heads = 12
    head_dim = hidden // num_heads
    inter = 4 * hidden

    log(f"Config L={L} hidden={hidden} heads={num_heads} head_dim={head_dim}")
    N = 1 << 16
    log(f"Init backend N={N}...")
    be = HEonGPUBackend(
        poly_modulus_degree=N,
        q_prime_bits=(60,) + (50,) * 28,
        p_prime_bits=(60, 60),
        scale_bits=50,
        sec_none=True,
    )
    log(f"  ready max_depth={be._max_depth} n_slots={be._num_slots}")
    n_new = prepare_colmajor_keys(be, L=L, max_dim=256)
    log(f"  +{n_new} keys")

    rng = np.random.default_rng(0)
    Wq = rng.standard_normal((hidden, hidden)) * 0.05
    Wk = rng.standard_normal((hidden, hidden)) * 0.05
    Wv = rng.standard_normal((hidden, hidden)) * 0.05
    Wo = rng.standard_normal((hidden, hidden)) * 0.05
    W1 = rng.standard_normal((inter, hidden)) * 0.05
    W2 = rng.standard_normal((hidden, inter)) * 0.05
    gamma1, beta1 = np.ones(hidden), np.zeros(hidden)
    gamma2, beta2 = np.ones(hidden), np.zeros(hidden)

    invsqrt_coeffs = [1.0, -0.5, 0.375]
    softmax_coeffs = [1.0, 0.5, 0.125]
    inv_sqrt_d = 1.0 / np.sqrt(head_dim)

    pt_cache: dict = {}
    attn_caches = [{} for _ in range(3)]  # one per ct group, persists across layers

    def run_layer(x_cts, layer_idx):
        layer_t = time.time()
        times = {}
        def stamp(name, dt):
            times[name] = times.get(name, 0.0) + dt

        t = time.time()
        Q = linear_colmajor_multi_streaming(
            be, x_cts, Wq, L=L, in_dim=hidden, out_dim=hidden,
            pt_cache=pt_cache, cache_tag="Wq",
        )
        stamp("Wq", time.time() - t)
        t = time.time()
        K = linear_colmajor_multi_streaming(
            be, x_cts, Wk, L=L, in_dim=hidden, out_dim=hidden,
            pt_cache=pt_cache, cache_tag="Wk",
        )
        stamp("Wk", time.time() - t)
        t = time.time()
        V = linear_colmajor_multi_streaming(
            be, x_cts, Wv, L=L, in_dim=hidden, out_dim=hidden,
            pt_cache=pt_cache, cache_tag="Wv",
        )
        stamp("Wv", time.time() - t)

        t = time.time()
        S = []
        for k in range(len(Q)):
            S.append(qk_scores_nexus(
                be, Q[k], K[k], L=L, head_dim=head_dim, scale=inv_sqrt_d,
                num_heads_per_ct=4, cache=attn_caches[k],
            ))
        stamp("qk", time.time() - t)
        del Q, K; gc.collect()

        t = time.time()
        Asm = [be.polyval(s, list(softmax_coeffs)) for s in S]
        stamp("softmax", time.time() - t)
        del S; gc.collect()

        t = time.time()
        O = []
        for k in range(len(V)):
            O.append(attn_apply_nexus(
                be, Asm[k], V[k], L=L, head_dim=head_dim,
                num_heads_per_ct=4, cache=attn_caches[k],
            ))
        stamp("av", time.time() - t)
        del V, Asm; gc.collect()

        t = time.time()
        Wo_out = linear_colmajor_multi_streaming(
            be, O, Wo, L=L, in_dim=hidden, out_dim=hidden,
            pt_cache=pt_cache, cache_tag="Wo",
        )
        stamp("Wo", time.time() - t)
        del O; gc.collect()

        t = time.time()
        res1 = add_multi(be, Wo_out, x_cts)
        stamp("residual1", time.time() - t)
        del Wo_out; gc.collect()

        t = time.time()
        h1 = layernorm_colmajor_multi(
            be, res1, L=L, hidden=hidden,
            invsqrt_power_coeffs=invsqrt_coeffs,
            invsqrt_interval=(-1.0, 1.0),
            gamma=gamma1, beta=beta1,
        )
        stamp("LN1", time.time() - t)
        del res1; gc.collect()

        t = time.time()
        H = linear_colmajor_multi_streaming(
            be, h1, W1, L=L, in_dim=hidden, out_dim=inter,
            pt_cache=pt_cache, cache_tag="W1",
        )
        stamp("W1", time.time() - t)

        t = time.time()
        G = [be.polyval(c, [0.5, 0.398, 0.0, -0.0066]) for c in H]
        stamp("gelu", time.time() - t)
        del H; gc.collect()

        t = time.time()
        H2 = linear_colmajor_multi_streaming(
            be, G, W2, L=L, in_dim=inter, out_dim=hidden,
            pt_cache=pt_cache, cache_tag="W2",
        )
        stamp("W2", time.time() - t)
        del G; gc.collect()

        t = time.time()
        res2 = add_multi(be, H2, h1)
        stamp("residual2", time.time() - t)
        del H2, h1; gc.collect()

        t = time.time()
        out = layernorm_colmajor_multi(
            be, res2, L=L, hidden=hidden,
            invsqrt_power_coeffs=invsqrt_coeffs,
            invsqrt_interval=(-1.0, 1.0),
            gamma=gamma2, beta=beta2,
        )
        stamp("LN2", time.time() - t)

        layer_dt = time.time() - layer_t
        log(f"--- Layer {layer_idx} TOTAL: {layer_dt:.2f}s ---")
        for k, v in sorted(times.items(), key=lambda kv: -kv[1]):
            log(f"     {k:<14s} {v*1000:8.0f} ms")
        return out, layer_dt

    x = rng.standard_normal((L, hidden)) * 0.1
    log("Pack input...")
    x_cts = pack_colmajor_multi(be, x, L=L, hidden=hidden)
    log(f"  {len(x_cts)} input cts at depth={be._ops.depth(x_cts[0])}")

    log("=== LAYER 1 (cold cache) ===")
    h_cts, t1 = run_layer(x_cts, layer_idx=1)

    # Layer 2 input must be at the same depth as layer 1 input (depth 0).
    # Re-encrypt to start fresh — in real BERT this would be the bootstrapped
    # output of layer 1; here it's a benchmark approximation.
    log("Re-encrypt for layer 2 (simulate bootstrap-back-to-fresh)...")
    h_arr = rng.standard_normal((L, hidden)) * 0.1
    h_cts2 = pack_colmajor_multi(be, h_arr, L=L, hidden=hidden)

    log("=== LAYER 2 (warm cache) ===")
    _, t2 = run_layer(h_cts2, layer_idx=2)

    print()
    print("=" * 70)
    print(f"L={L} MULTI-CT 2-LAYER  hidden={hidden}  N={N}")
    print(f"  Layer 1 (cold cache) : {t1:.2f}s")
    print(f"  Layer 2 (warm cache) : {t2:.2f}s")
    proj_e2e = t1 + 11 * t2
    print(f"  Projected 12-layer   : {proj_e2e:.1f}s "
          f"(NEXUS L=128 reported: 37.3s)")
    print("=" * 70)


if __name__ == "__main__":
    main()
