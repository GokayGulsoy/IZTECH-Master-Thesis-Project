...
\begin{enumerate}
    \item \textbf{Distribution-Weighted Minimax Polynomial Approximation.} A novel approximation method that incorporates the empirical activation distribution into the minimax objective, concentrating approximation effort in high-density regions. This achieves $3.4\times$ lower weighted error than standard Chebyshev approximation on GELU at degree~8.

    \item \textbf{Layer-Adaptive CKKS Depth Allocation.} A dynamic programming algorithm that optimally distributes a global multiplicative depth budget across layers and operations, achieving $25.7\times$ lower total weighted error compared to uniform allocation at depth budget $D = 15$.

    \item \textbf{Evolutionary Optimisation of Polynomial Parameters.} A genetic algorithm that jointly optimises polynomial degree assignments and approximation interval widths across all layers using KL divergence as a fitness metric, capturing inter-layer error interactions that independent-layer methods miss.

    \item \textbf{Depth-Optimal Polynomial Evaluation Strategies.} A systematic comparison of Horner, balanced binary tree, and Paterson--Stockmeyer evaluation strategies under CKKS encryption, demonstrating $7.1\times$ latency reduction with balanced tree evaluation.

    \item \textbf{Error Propagation Bounds for Polynomial-Approximated Transformers.} Formal bounds proving geometric error growth through sequential Transformer layers with amplification factor $\alpha \approx 6.16$ per layer, providing theoretical guidance for minimum polynomial degree requirements.
\end{enumerate}

All contributions are validated through a complete end-to-end pipeline spanning activation profiling, polynomial fitting, model surgery, approximation-aware fine-tuning, and CKKS encrypted inference on multiple BERT model variants (Tiny through Base) and the SST-2 sentiment classification benchmark.
...
\begin{itemize}
    \item \textbf{Chapter~\ref{ch:background}} provides background on the CKKS homomorphic encryption scheme, the non-linearity bottleneck in Transformers, and existing secure inference frameworks (CryptoNets, Iron, THE-X, BOLT).

    \item \textbf{Chapter~\ref{ch:methodology}} presents the proposed methodology, including the five contributions: distribution-weighted minimax approximation, layer-adaptive depth allocation, evolutionary optimisation, depth-optimal polynomial evaluation, and error propagation bounds.

    \item \textbf{Chapter~\ref{ch:results}} reports experimental results, including activation profiling, approximation quality, depth allocation, fine-tuning accuracy, CKKS encrypted inference benchmarks, multi-model scaling analysis, and comparison with existing frameworks.

    \item \textbf{Chapter~\ref{ch:conclusion}} summarises the contributions, discusses limitations, presents planned extensions---GPU-accelerated CKKS inference and bootstrapping for larger models---and outlines longer-term research directions.
\end{itemize}