...
A \emph{second}, equally important gap is left untouched even by recent pure-FHE Transformer systems such as NEXUS~\cite{zhang2024nexus} (CRYPTO'24) and the multi-GPU compiler/runtime CERIUM~\cite{cerium2025} (December 2025): the standard self-attention mechanism still requires $\Theta(L^2)$ ciphertext-by-ciphertext multiplications for the score matrix $Q\!\cdot\!K^\top$ and a deep polynomial Softmax on top of it---together accounting for the majority of the per-layer wall-time. CERIUM attacks this with framework-level engineering on 8$\times$B200 GPUs; NEXUS keeps the standard architecture and absorbs the cost. Neither modifies the architecture itself.
...
The primary challenge addressed in this thesis is twofold. First, the complete replacement of all non-polynomial operations in the Transformer architecture with learnable polynomial approximations that can be evaluated under CKKS encryption, while maintaining competitive accuracy with the original model. Second, the architectural reduction of the dominant ciphertext-by-ciphertext cost of self-attention so that the resulting circuit becomes practical on a \emph{single} GPU. The core research questions are:

\begin{quote}
\emph{(i) How can all three non-linear operations in Transformers---GELU, Softmax, and LayerNorm---be replaced by learnable polynomial approximations using a progressive training strategy, such that the resulting fully-polynomial model achieves accuracy competitive with the original while enabling non-interactive encrypted inference? (ii) Can the standard self-attention block itself be replaced by an FHE-friendly variant that eliminates the data-dependent $Q\!\cdot\!K^\top$ ciphertext-by-ciphertext multiplications and the polynomial Softmax, while preserving GLUE-level accuracy and pushing single-GPU end-to-end FHE BERT inference into the sub-100-second regime?}
\end{quote}
...
\begin{enumerate}
    ...
    \item \textbf{Synthesizer-LPAN: Architectural Elimination of $Q$, $K$, and Softmax under FHE.} An architectural extension that replaces the standard self-attention block with a frozen, learned, plaintext attention pattern $A \in \mathbb{R}^{L\times L}$ in the spirit of Tay et al.~\cite{tay2020synthesizer}. Under FHE this eliminates the $W_q$ and $W_k$ projections, the $Q\!\cdot\!K^\top$ ciphertext-by-ciphertext multiplications, and the depth-12 polynomial Softmax \emph{entirely}. Combined with Halevi--Shoup baby-step giant-step (BSGS) fusion of the cyclic-shift mask with each plaintext diagonal~\cite{halevi2014bsgs} ($2L \to 2\sqrt{L}$ rotations), batched LPAN polynomial evaluation ($N_{\mathrm{BATCH}}{=}16$), and a tuned CKKS chain budget ($\mathrm{chain}{=}22$), Synthesizer-LPAN delivers a \emph{single-GPU sub-100-second end-to-end coherent FHE BERT result on plain HEonGPU}: \textbf{60.9\,s} for a 12-layer forward pass at $L{=}128$ on a single H100, a $\mathbf{13.67\times}$ speed-up over the honest LPAN baseline ($833$\,s), measured on the vendored HEonGPU~\cite{kim2024heongpu} CKKS backend. Concurrent Cerium results are faster on the same hardware via compiler/runtime optimization; our contribution is therefore architectural complementarity rather than current latency SOTA.
\end{enumerate}

All contributions are validated through a complete end-to-end pipeline on four BERT model variants (Tiny, Mini, Small, and Base) across two GLUE tasks: SST-2 sentiment classification and MRPC paraphrase detection. Smaller models (Tiny, Mini, Small) were trained on a local NVIDIA RTX 5070~Ti laptop GPU (12~GB VRAM, CUDA~12.8); BERT-Base was trained on a RunPod NVIDIA H100 SXM pod (80~GB HBM3, 28-vCPU Intel Xeon Platinum 8480+, CUDA~13.0). The LPAN framework achieves:
\begin{itemize}
    \item \textbf{BERT-Tiny (SST-2):} 83.26\% $\to$ 83.14\% (\textbf{0.11\% drop})
    \item \textbf{BERT-Mini (SST-2):} 87.16\% $\to$ 86.81\% (\textbf{0.34\% drop})
    \item \textbf{BERT-Small (SST-2):} 87.73\% $\to$ 88.53\% (\textbf{0.80\% gain})
    \item \textbf{BERT-Base (SST-2):} 93.12\% $\to$ 91.40\% (\textbf{1.72\% drop}, beats MPCFormer 91.3\%)
    \item \textbf{BERT-Base (MRPC):} 83.82\% acc $\to$ 82.35\% acc / 87.71\% F1 (\textbf{1.47\% drop}, beats MPCFormer 82.1\%)
\end{itemize}
These results demonstrate that LPAN is the only framework to replace \emph{all three} non-linear operations---including the previously untouched LayerNorm---while achieving accuracy competitive with or exceeding MPCFormer, which replaces only GELU and Softmax via multi-party computation.
...
\begin{itemize}
    ...
    \item \textbf{Chapter~\ref{ch:methodology}} presents the proposed LPAN methodology, including distribution-weighted minimax polynomial initialisation, the three-stage progressive replacement pipeline, attention-level knowledge distillation, per-head polynomial Softmax, polynomial LayerNorm, and error propagation analysis. It then introduces the \emph{Synthesizer-LPAN} architectural extension, the BSGS-fused diagonal decomposition of the plaintext attention pattern, and the batched packed CKKS evaluation strategy that delivers the headline 60.9\,s single-GPU result.

    \item \textbf{Chapter~\ref{ch:results}} reports experimental results across four BERT model variants, including activation profiling, approximation quality, progressive training analysis for BERT-Base, comparison with existing frameworks, and CKKS encrypted inference benchmarks. A dedicated section presents the Synthesizer-LPAN end-to-end FHE benchmark, the wall-time ladder of optimisations (LPAN baseline $\to$ Synthesizer-LPAN+BSGS), and a positioning table against NEXUS, CERIUM, BOLT, Iron and MPCFormer.
