...
### Step 1 — Profile activations (one-time, model-only)

```bash
python experiments/run_analysis.py profile
```

Writes `results/activation_profiles/` (PNGs + per-layer KDE samples
for GELU inputs / Softmax pre-shift / LN variances). This populates
the `PROFILED_INTERVALS` defaults that the polynomial fits use.

> **Already done for BERT-Tiny.** Re-run only if you change the
> dataset or want per-layer profiles for Mini/Small/Base.
...
Pulls JSON/PNG from every `results/<sub>/` dir and writes the final
publication-ready figures into `results/figures/`. Drop those into
