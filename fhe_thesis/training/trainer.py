...
class AttentionDistillationTrainer(NaNSafeTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        for l in range(num_layers):
            # Skip layers that do not emit explicit attention tensors.
        ...
        # Clamp AttnKL because attention-family changes can produce rare spikes
        ...
        # Clamp HidMSE to prevent rare pathological batches from dominating the
        # update with unusually large hidden-state deviations.
...
class SynthesizerDistillationTrainer(NaNSafeTrainer):
    """Trainer for Stage 4 Synthesizer attention distillation.

    Uses the current Stage-3 LPAN model as the frozen teacher and trains a
    student whose self-attention blocks are replaced with learned fixed-pattern
    Synthesizer attention modules.

    Loss: ``L = CE + lambda(t) * KL(teacher_attn || student_attn)`` where the
    distillation weight is annealed linearly from ``distill_weight_start`` to
    ``distill_weight_end`` over the run.
    """

    def __init__(
        self,
        teacher_model: nn.Module,
        distill_weight_start: float = 1.0,
        ...
        **kwargs,
        ...
        for p in self.teacher.parameters():
            p.requires_grad = False
        self.distill_weight_start = distill_weight_start
        self.distill_weight_end = distill_weight_end

    def _current_distill_weight(self) -> float:
        total_steps = max(getattr(self.state, "max_steps", 0), 0)
        current_step = max(getattr(self.state, "global_step", 0), 0)
        if total_steps <= 0:
            return float(self.distill_weight_start)
        progress = min(current_step / max(total_steps, 1), 1.0)
        return float(
            self.distill_weight_start
            + progress * (self.distill_weight_end - self.distill_weight_start)
        )
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        student_attns = outputs.attentions
        teacher_attns = teacher_out.attentions

        attn_loss = torch.tensor(0.0, device=outputs.logits.device)
        num_layers = min(len(student_attns), len(teacher_attns))
        attn_count = 0
        for layer_idx in range(num_layers):
            if student_attns[layer_idx] is None:
                continue
            s_attn = student_attns[layer_idx].float().clamp(min=1e-8)
            t_attn = teacher_attns[layer_idx].float().clamp(min=1e-8)
            batch, heads, seq_len, _ = s_attn.shape
            attn_loss = attn_loss + F.kl_div(
                s_attn.log(), t_attn, reduction="sum"
            ) / (batch * heads * seq_len)
            attn_count += 1
        ...
        loss = task_loss + distill_weight * attn_loss

        if hasattr(self, "state") and self.state.global_step % 100 == 0:
            print(
                f"  [SynthKD step {self.state.global_step}] "
                f"CE={task_loss.item():.4f} "
                f"AttnKL={attn_loss.item():.4f} "
                f"lambda={distill_weight:.4f} "
                f"total={loss.item():.4f}"
            )
...
def synth_attn_distill_and_eval(
    student_model: nn.Module,
    teacher_model: nn.Module,
    train_dataset,
    eval_dataset,
    ...
    """Fine-tune Stage 4 Synthesizer attention with annealed attention KD."""
    device = detect_device()
    ...
    trainer = SynthesizerDistillationTrainer(
        teacher_model=teacher_model,
        distill_weight_start=distill_weight_start,
        distill_weight_end=distill_weight_end,
        model=student_model,
        args=args,
        train_dataset=train_dataset,
        eval_dataset=eval_dataset,
    ...
    print(
        f"  Training '{label}' for {epochs} epochs "
        f"(SynthKD: lambda {distill_weight_start:.2f}->{distill_weight_end:.2f}, "
        f"bs={batch_size}, lr={lr})..."
    )
    trainer.train()
    results = trainer.evaluate()
