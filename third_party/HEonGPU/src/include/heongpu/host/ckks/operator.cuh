...
namespace heongpu
{
    ...
    class HEArithmeticOperator<Scheme::CKKS> : public HEOperator<Scheme::CKKS>
    {
      public:
        ...
        // Accessors for Python bindings (NEXUS-style coefficient packing
        // pipelines need to know the bootstrap scale + CtoS chain level
        // to align ciphertext scale/level before calling coeff_to_slot
        // outside of the regular_bootstrapping fast path).
        __host__ double bootstrapping_scale() const { return scale_boot_; }
        __host__ int    coeff_to_slot_level() const { return CtoS_level_; }
        __host__ int    slot_to_coeff_level() const { return StoC_level_; }
        __host__ bool   bootstrapping_ready() const { return boot_context_generated_; }
    };
