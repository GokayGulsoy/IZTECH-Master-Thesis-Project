...
namespace heongpu
{
    ...
    template <> class Ciphertext<Scheme::CKKS>
    {
      ...
      public:
        ...
        // NEXUS-style escape hatch: caller asserts the next operation
        // will absorb the doubled scale (e.g. CtoS internally rescales
        // twice). Used by Python bindings to chain mul_plain → CtoS
        // without a depth-consuming intermediate rescale.
        inline void clear_rescale_required() noexcept
        {
            rescale_required_ = false;
        }
        inline void set_rescale_required() noexcept
        {
